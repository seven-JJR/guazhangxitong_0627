...
import openpyxl
user_to_name={'D18030023':'蒋佳芮','Q21070534':'饶昌银','Q21070526':'左丹','Q21071566':'尹小娇','Q21091346':'万方林','Q21091383':'冉亚东','Q21091427':'胡光荣','Q21112943':'肖光祥',\
              'Q21115503':'李继兰','Q21121027':'王浪','Q21123050':'李俊淑','Q21123386':'魏天明','Q22010545':'王大贵','Q22023630':'蒲云林','Q22070577':'易洁','Q17110507':'舒锐',\
              'Q18060537':'王春龙','Q18070521':'邢晓娇','Q21091451':'吴昊','Q15120012':'汤勇','Q18040522':'奚茜','Q18070522':'王春艳','Q18070530':'陈云华','Q18070540':'雷双玉',\
              'Q18070542':'张月','Q14085182':'谭文龙','Q12120021':'方伟'}
gonghao_all=['D18030023','Q21070534','Q21070526','Q21071566','Q21091346','Q21091383','Q21091427','Q21112943','Q21115503','Q21121027','Q21123050','Q21123386','Q22010545','Q22023630','Q22070577','Q17110507','Q18060537','Q18070521','Q21091451','Q15120012','Q18040522','Q18070522','Q18070530','Q18070540','Q18070542','Q14085182','Q12120021']
user=''
#---------------登录界面----------------------
#---------------这一部分是UI逻辑分离固定格式----------------------
...
# ---------------这一部分是设置自定义窗口后，app不能拖动，重启可拖动的方式---------------------
    ...
    def denglu(self):
        ...
        try:
            db = pymysql.connect(host="172.17.1.71", user='root', password='jiang111', port=3306, database='lily', connect_timeout=3)
            cusor = db.cursor()
...
# ---------------这一部分是设置自定义窗口后，app不能拖动，重启可拖动的方式---------------------
    ...
    def chaxun_page1(self):#查询只设置3种情况，要么2个标签都有内容，要么其中一个有，否则查询函数为pass
        ...
        if self.lineEdit.text()!='' and self.lineEdit_2.text()=='':#project name不为空
            ...
            # self.guazhang=(('LCH-2', '4X90S91831', 'CABLE USB-C TO ETHERNET ADP SC10P27625', '2022012111770'),('LCH-1', '20Y6Z9EBCN', 'THINKPAD X1 EXTREME GEN 4 SVT19 PRC', '2022012621453'))
            for each in self.jiechu_result_page1:
                if self.lineEdit.text() in each[0] and self.lineEdit_2.text() in each[2]:  # 如果搜索的名字被包含在PARTname里
    ...
    def page1_jiqi_guihuan_queren(self):# 15——label是记录button编号的label标签，颜色和背景一样看不出来哈哈
        ...
        self.label_654.setText('')
        try:
            db = pymysql.connect(host="172.17.1.71", user='root', password='jiang111', port=3306, database='lily', connect_timeout=4)
            cusor = db.cursor()
            try:
